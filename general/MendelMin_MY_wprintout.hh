#ifndef MY_MendelMin_H
#define MY_MendelMin_H

// removed Rivet's own random number generator from here
...
using std::valarray;
  ...
  public:
    ...
    float rnd() const {
      //return rand01(); // <-- Rivet's own rand function
      float cxx_rand = rand(); ///RAND_MAX;
      return cxx_rand/RAND_MAX;
    }
	...
	std::cout << "min result: " << mm.first << std::endl;
	std::cout << "max result: " << mm.second << std::endl;
      }
	...
	std::cout << "swapping lowest result (and it's associated paramters!) to position zero (" << iwin << "<->0)" << std::endl;
