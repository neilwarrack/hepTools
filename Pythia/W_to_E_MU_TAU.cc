...
namespace Pythia8 {
  ...
  struct W_to_E_MU_TAU : public UserHooks {
	...
	//       	cout << " wd1.id()= " << wd1.id() << ", wd2.id()= " << wd2.id() << endl; 

	if ( wd1.idAbs() == 11 || wd1.idAbs() == 13 || wd1.idAbs() == 15 || 
	     wd2.idAbs() == 11 || wd2.idAbs() == 13 || wd2.idAbs() == 15 ) { 
	  nE_MU_TAU += 1;}// cout << "nE_MU_TAU+=1" << endl ;}
	
	else if ( wd1.isQuark()  && wd2.isQuark()  ) { 
	  nH += 1; 
	  //cout << "nH+=1" << endl ;
	} else {
	  cout << "HOOK: Needs investigation! Non hadronic and non leptonic W decay!?!?" << endl;
	}
  ...
  Sacrifice::UserHooksFactory::Creator<Pythia8::W_to_E_MU_TAU> TTOEMUTAU("W_to_E_MU_TAU");
